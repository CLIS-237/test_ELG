...
def split_data(hg, etype_name, train_ratio, eval_ratio, test_ratio, usage):
    ...
    pos_unused_data = pos_data[pos_train_size + pos_eval_size + pos_test_size:]
    ...
    eval_data = np.array(eval_data)
    test_data = np.array(test_data)
    pos_unused_data = np.array(pos_unused_data)
    
    return train_data, eval_data, test_data, pos_unused_data
